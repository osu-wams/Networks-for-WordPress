...
if ( ! function_exists( 'add_site' ) ) {
	...
	function add_site( $domain, $path = '/', $blog_name = NULL, $cloneSite = NULL, $options_to_clone = NULL ) {
		...
		$query = "SELECT * FROM {$wpdb->site} WHERE domain='" . esc_sql( $domain ) . "' AND path='" . esc_sql( $path ) . "' LIMIT 1";
				...
				if ( $optionsCache[ $option ] !== FALSE ) {
					...
					if ( $option == 'ms_files_rewriting' ) {
						$wpdb->insert( $wpdb->sitemeta, [
							'site_id'    => $wpdb->siteid,
							'meta_key'   => $option,
							'meta_value' => $optionsCache[ $option ],
						] );
					} else {
...
if ( ! function_exists( 'update_site' ) ) {
	...
	function update_site( $id, $domain, $path = '', $meta = NULL ) {
		...
		do_action( 'wpmu_update_site', $id, [
			'domain' => $site->domain,
			'path'   => $site->path,
		] );
		do_action( 'wpms_update_network', $id, [
			'domain' => $site->domain,
			'path'   => $site->path,
		] );
...
if ( ! function_exists( 'is_super_admin_for' ) ) {
	...
	function is_super_admin_for( $site_ids = NULL ) {
		...
		foreach ( $site_ids as $site_id ) {
			...
			if ( ! $is_super_admin ) {
				break;
			} // no need to keep looking once one fails
		}
