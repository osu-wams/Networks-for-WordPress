...
class wp_Networks_Admin {
	...
	public function __construct() {
		...
		add_action( 'network_admin_menu', [ &$this, 'networks_admin_menu' ] );

		add_filter( 'manage_sites_action_links', [
			&$this,
			'add_blogs_link',
		], 10, 3 );

		/** Compatibility with older releases */
		add_action( 'admin_menu', [ &$this, 'networks_admin_menu' ] );
		if ( ! has_action( 'manage_sites_action_links' ) ) {
			add_action( 'wpmublogsaction', [ &$this, 'assign_blogs_link' ] );
		}

		wp_register_script( 'njsl_networks_admin_js', plugins_url( '/_inc/admin.js', __FILE__ ), [ 'jquery' ] );
		...
		add_filter( 'manage_sitemeta_descriptions', [
			&$this,
			'add_sitemeta_descriptions',
		] );
	}

	public function assign_blogs_link( $blog_id = NULL ) {
		global $blog;
		if ( $blog_id == NULL ) {
	...
	public function add_blogs_link( $actions, $blog_id, $blog_name ) {
		$actions['move'] = '<a href="' . $this->listPage . '&amp;action=move&amp;blog_id=' . $blog_id . '" class="edit">' . __( 'Move', 'njsl-networks' ) . '</a>';

		return $actions;
	}

	public function networks_admin_menu() {
		if ( function_exists( 'is_network_admin' ) ) {
			/** WP >= 3.1 */
			$this->admin_page = add_submenu_page( 'sites.php', __( 'Networks', 'njsl-networks' ), __( 'Networks', 'njsl-networks' ), 'manage_network_options', $this->slug, [
				&$this,
				'sites_page',
			] );
			...
			/** WP < 3.1    */
			$this->admin_page = add_submenu_page( 'ms-admin.php', __( 'Networks', 'njsl-networks' ), __( 'Networks', 'njsl-networks' ), 'manage_options', $this->slug, [
				&$this,
				'sites_page',
			] );
		...
		add_action( 'load-' . $this->admin_page, [
			&$this,
			'networks_help_screen',
		] );
		add_action( 'load-' . $this->admin_page, [
			&$this,
			'networks_options_screen',
		] );
		add_action( 'admin_print_scripts-' . $this->admin_page, [
			&$this,
			'add_admin_scripts',
		] );
		add_action( 'admin_print_styles-' . $this->admin_page, [
			&$this,
			'add_admin_styles',
		] );
	}
	...
	public function add_admin_scripts() {
		wp_enqueue_script( 'wp-pointer', FALSE, [ 'jquery' ] );
		wp_enqueue_script( 'njsl_networks_admin_js', FALSE, [ 'jquery' ] );
	...
	public function sites_page() {
		...
		if ( isset( $_GET['updated'] ) ) {
            ...
            <p><?php _e( 'Options saved.', 'njsl-networks' ) ?></p></div><?php
		} else if ( isset( $_GET['added'] ) ) {
			?>
            <div id="message" class="updated fade">
            <p><?php _e( 'Network created.', 'njsl-networks' ); ?></p>
            </div><?php
		} else if ( isset( $_GET['deleted'] ) ) {
			?>
            <div id="message" class="updated fade">
            <p><?php _e( 'Network(s) deleted.', 'njsl-networks' ); ?></p>
            </div><?php
		}
			...
			default:
				...
				if ( isset( $_GET['s'] ) ) {
					if ( isset( $_GET['search'] ) && $_GET['search'] == __( 'Search Networks', 'njsl-networks' ) ) {
						$countSearchConditions = ', ' . $wpdb->sitemeta . ' WHERE ' . $wpdb->site . '.domain LIKE ' . "'%" . esc_sql( $_GET['s'] ) . "%'";
						$countSearchConditions .= 'OR ( ' . $wpdb->sitemeta . '.site_id = ' . $wpdb->site . '.id AND ' . $wpdb->sitemeta . '.meta_value LIKE ' . "'%" . esc_sql( $_GET['s'] ) . "%')";
					}
				}
				...
				if ( isset( $_GET['s'] ) ) {
					if ( isset( $_GET['search'] ) && $_GET['search'] == __( 'Search Networks', 'njsl-networks' ) ) {
						$searchConditions = ' WHERE ' . 'wp_site.domain LIKE ' . "'%" . esc_sql( $_GET['s'] ) . "%'";
						$searchConditions .= ' OR ' . 'sm1.meta_value LIKE ' . "'%" . esc_sql( $_GET['s'] ) . "%'";
						$searchConditions .= ' OR ' . 'sm2.meta_value LIKE ' . "'%" . esc_sql( $_GET['s'] ) . "%'";
					}
				}
                ...
                <div class="wrap">
                    ...
                    <h2><?php _e( 'Networks', 'njsl-networks' ) ?> <a
                                href="<?php echo add_query_arg( [ 'page' => 'networks' ] ); ?>#form-add-network"
                                class="button add-new-h2"><?php _e( 'Add New' ) ?></a>
                    </h2>
                    <form name="searchform"
                          action="<?php echo add_query_arg( [ 'page' => 'networks' ] ); ?>"
                          method="get">
                        <p class="search-box">
                            <label class="screen-reader-text"
                                   for="network-search-input"><?php _e( 'Search Networks', 'njsl-networks' ); ?>
                                :</label>
                            ...
                            <input type="submit" name="search" id="search"
                                   class="button"
                                   value="<?php _e( 'Search Networks', 'njsl-networks' ); ?>"/>
                        </p>
                    </form>
					<?php
					...
					?>
                    <form name='formlist'
                          action='<?php echo add_query_arg( [ 'action' => 'delete_multisites' ] ); ?>'
                          method='POST'>
                        <div class="tablenav">
							<?php if ( $network_nav ) {
								echo "<div class='tablenav-pages'>$network_nav</div>";
							} ?>
                            <div class="alignleft">
                                <input type="submit"
                                       ...
                                       value="<?php _e( 'Delete' ); ?>"/>
								<?php if ( isset( $_GET['s'] ) ) { ?>
                                    <p><?php _e( 'Filter', 'njsl-networks' ); ?>
                                        :
                                        <a href="<?php echo $this->listPage ?>"
                                           title="<?php _e( 'Remove this filter', 'njsl-networks' ) ?>"><?php echo esc_sql( $_GET['s'] ) ?></a>
                                    </p>
								<?php } ?>
                            </div>
                        </div>
								...
								<?php foreach ( $networks_columns as $col_name => $column_display_name ) { ?>
                                    <th scope="col">
										<?php if ( in_array( $col_name, $sortable_networks_columns ) ) : ?>
                                            <a href="<?php echo $this->listPage ?>&sortby=<?php echo urlencode( $column_display_name ) ?>&<?php if ( $_GET['sortby'] == $column_display_name ) {
												if ( $order == 'DESC' ) {
													echo "order=ASC&";
												} else {
													echo "order=DESC&";
												}
											} ?>start=<?php echo $start ?>"><?php echo $column_display_name; ?></a>
										<?php else: ?>
										...
										switch ( $column_name ) {
											...
											case 'id':
                                                ...
                                                <th scope="row"
                                                    ...
                                                    <input type='checkbox'
                                                           id='<?php echo $blog['id'] ?>'
                                                           name='allsites[]'
                                                           value='<?php echo $blog['id'] ?>'<?php if ( $blog['id'] == 1 ) {
														echo 'disabled';
													} ?>>
                                                </th>
											...
											case 'site':
                                                ...
                                                <td valign='top'>
													...
													$actions = [
														'assign_sites' => '<span class="edit"><a href="' . add_query_arg( [
																'action' => 'assignblogs',
																'id'     => $blog['id'],
															], $queryStr ) . '" title="' . __( 'Assign sites to this network', 'njsl-networks' ) . '">' . __( 'Assign Sites', 'njsl-networks' ) . '</a></span>',
														'edit'         => '<span class="edit"><a class="edit_network_link" href="' . add_query_arg( [
																'action' => 'editsite',
																'id'     => $blog['id'],
															], $queryStr ) . '" title="' . __( 'Edit this network', 'njsl-networks' ) . '">' . __( 'Edit', 'njsl-networks' ) . '</a></span>',
														'verify'       => '<span class="edit"><a class="verify_network_link" href="' . add_query_arg( [
																'action' => 'verifynetwork',
																'id'     => $blog['id'],
															], $queryStr ) . '" title="' . __( 'Check this network for configuration errors', 'njsl-networks' ) . '">' . __( 'Verify', 'njsl-networks' ) . '</a></span>',
														'delete'       => '<span class="delete"><a href="' . add_query_arg( [
																'action' => 'deletesite',
																'id'     => $blog['id'],
															], $queryStr ) . '" title="' . __( 'Delete this network', 'njsl-networks' ) . '">' . __( 'Delete', 'njsl-networks' ) . '</a></span>',
													];
											...
											case 'path':
												?>
                                                <td valign='top'><label
                                                            for='<?php echo $blog['id'] ?>'><?php echo $blog['path'] ?></label>
                                                </td>
												<?php
												break;
											case 'sites':
												?>
                                                <td valign='top'><a
                                                            href="<?php echo( is_ssl() ? 'https' : 'http' ) ?>://<?php echo $blog['domain'] . $blog['blog_path']; ?>wp-admin/<?php echo ( strpos( $this->listPage, 'site' ) !== FALSE ) ? 'network/' . $this->sitesPage : $this->sitesPage ?>"
                                                            title="<?php _e( 'Go to this network to manage its sites', 'njsl-networks' ); ?>"><?php echo $blog['blogs'] ?></a>
                                                </td>
												<?php
												break;
											case 'admins':
                                                ...
                                                <td valign='top'>
													<?php foreach ( $admins as $admin ) : ?>
														...
														if ( $this_admin ) : ?>
                                                            <a href="<?php echo esc_url( network_admin_url( add_query_arg( 'wp_http_referer', urlencode( stripslashes( $_SERVER['REQUEST_URI'] ) ), 'user-edit.php?user_id=' . $this_admin->ID ) ) ) ?>"><?php echo $this_admin->user_login ?></a>
                                                            <br/>
														<?php
														endif;
													endforeach; ?>
                    ...
                    <h3>
                        <a name="form-add-network"></a><?php _e( 'Create a Network', 'njsl-networks' ); ?>
                    </h3>
                    <form method="POST"
                          action="<?php echo add_query_arg( [ 'action' => 'addsite' ] ); ?>">
                            ...
                            <tr>
                                <td>
                                    <table class="form-table">
                                        <tr>
                                            <th scope="row"><label
                                                        for="newName"><?php _e( 'Network Name', 'njsl-networks' ); ?>
                                                    :</label></th>
                                            <td><input type="text" name="name"
                                                       id="newName"
                                                       placeholder="<?php _e( 'My new network', 'njsl-networks' ); ?>"
                                                       title="<?php _e( 'A friendly name for your new Network', 'njsl-networks' ); ?>"/>
                                            </td>
                                        </tr>
										<?php if ( is_subdomain_install() ) : ?>
                                            <tr>
                                                <th scope="row"><?php _e( 'Network URL', 'njsl-networks' ) ?>
                                                    :
                                                </th>
                                                <td>
                                                    <table>
                                                        <tr>
                                                            <th scope="row"
                                                                class="network-uri-th">
                                                                <label for="newDom"><?php _e( 'Domain', 'njsl-networks' ); ?>
                                                                    :</label>
                                                            </th>
                                                            <th scope="row"
                                                                class="network-uri-th">
                                                                <label for="newPath"><?php _e( 'Path', 'njsl-networks' ); ?>
                                                                    :</label>
                                                            </th>
                                                        </tr>
                                                        <tr>
                                                            <td> http://<input
                                                                        ...
                                                                        name="domain"
                                                                        id="newDom"
                                                                        title="<?php _e( 'The domain for your new Network', 'njsl-networks' ); ?>"/>
                                                            </td>
                                                            <td>
                                                                <code>/</code><input
                                                                        type="hidden"
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <input type="text"
                                                                       ...
                                                                       id="newPath"
                                                                       placeholder="/"
                                                                       title="<?php _e( 'If you are unsure, put in /' ); ?>"/>
                                                            </td>
                                                        </tr>
                                                    </table>
                                                </td>
                                        ...
                                        <tr>
                                            <th scope="row"><label
                                                        for="create_root_site"><?php _e( 'Create a Main Site', 'njsl-networks' ) ?>
                                                    :</label></th>
                                        ...
                                        <tr>
                                            <th scope="row"><label
                                                        for="newBlog"><?php _e( 'Main Site Name', 'njsl-networks' ); ?>
                                                    :</label></th>
                                            <td><input type="text"
                                                       name="newBlog"
                                                       id="newBlog"
                                                       placeholder="<?php _e( 'My new site name', 'njsl-networks' ); ?>"
                                                       title="<?php _e( 'The name for the new Network\'s main site', 'njsl-networks' ); ?>"/>
                                            </td>
                                        </tr>
                                    </table>
                                </td>
                                <td style="vertical-align: top"
                                    id="new-network-preview">
                                    <h4>New Network Preview</h4>
                                    <ul>
                                        <li>Domain: <span id="domain_preview"/>
                                        </li>
                                        <li>Site URL: <span
                                                    id="siteurl_preview"/></li>
                                    </ul>
                                    <input type="button" name="verify_domain"
                                           id="verify_domain"
                                           value="<?php _e( 'Check Network Settings', 'njsl-networks' ); ?>"/>
                                    <div id="verifying"/>
                                </td>
                            </tr>
                        </table>
                        <div class="metabox-holder meta-box-sortables"
                             id="advanced_site_options">
                            <div class="postbox if-js-closed">
                                <div title="Click to toggle" class="handlediv">
                                    <br/></div>
                                <h3>
                                    <span><?php _e( 'Advanced Network Options', 'njsl-networks' ); ?></span>
                                </h3>
                                <div class="inside">
                                    <table class="form-table">
                                        <tr>
                                            <th scope="row"><label
                                                        for="cloneSite"><?php _e( 'Copy Network Options From', 'njsl-networks' ); ?>
                                                    :</label></th>
                                            ...
                                            <td colspan="2"><select
                                                        name="cloneSite"
                                                        id="cloneSite">
                                                    <option value="0"><?php _e( 'Do Not Copy', 'njsl-networks' ); ?></option><?php foreach ( $site_list as $site ) {
														echo '<option value="' . $site['id'] . '"' . ( $site['id'] == 1 ? ' selected' : '' ) . '>' . $site['site_name'] . ' (' . $site['domain'] . ')' . '</option>';
													} ?></select></td>
                                        </tr>
                                        <tr>
                                            <th scope="row" valign="top">
                                                <label><?php _e( 'Options to Copy', 'njsl-networks' ); ?>
                                                    :</label></th>
                                            <td>
                                            </td>
                                            <td valign="top">
													...
													<?php foreach ( $all_site_options as $count => $option ) { ?>
                                                        <tr class="<?php echo $class = ( 'alternate' == $class ) ? '' : 'alternate'; ?>">
                                                            <th scope="row"
                                                                class="check-column">
                                                                <input type="checkbox"
                                                                       id="option_<?php echo $count; ?>"
                                                                       name="options_to_clone[<?php echo $option->meta_key; ?>]"<?php echo( in_array( $option->meta_key, $options_to_copy ) ? ' checked' : '' ); ?> />
                                                            </th>
                                                            <td>
                                                                <label for="option_<?php echo $count; ?>"
                                                                       title="<?php echo $option->meta_key ?>"><?php echo( strlen( $option->meta_key ) > 35 ? substr( $option->meta_key, 0, 35 ) . '...' : $option->meta_key ); ?></label>
                                                            </td>
                                                            <td>
                                                                <label for="option_<?php echo $count; ?>"><?php echo( array_key_exists( $option->meta_key, $known_sitemeta_options ) ? __( $known_sitemeta_options[ $option->meta_key ] ) : '' ); ?></label>
                                                            </td>
                                                        </tr>
													<?php } ?>
                                                    </tbody>
	...
	function edit_site_page() {
		...
		if ( isset( $_POST['update'] ) && isset( $_GET['id'] ) ) {
			...
			$queryStr = add_query_arg( [ 'action' => FALSE ] );

			?>
            <div class="wrap">
                <div class="icon32" id="icon-ms-admin"><br></div>
                <h2><?php _e( 'Edit Network', 'njsl-networks' ); ?>: <a
                            href="http://<?php echo $site->domain . $site->path ?>"><?php echo $site->domain . $site->path ?></a>
                </h2>
                <form method="post" action="<?php echo $queryStr; ?>">
                    <table class="form-table">
                        <tr class="form-field">
                            <th scope="row"><label
                                        for="domain"><?php _e( 'Domain', 'njsl-networks' ); ?></label>
                            </th>
                            <td> http://<input type="text" id="domain"
                                               name="domain"
                                               value="<?php echo $site->domain; ?>">
                            </td>
                        </tr>
                        <tr class="form-field">
                            <th scope="row"><label
                                        for="path"><?php _e( 'Path', 'njsl-networks' ); ?></label>
                            </th>
                            <td><input type="text" id="path" name="path"
                                       value="<?php echo $site->path; ?>"/></td>
                        </tr>

						<?php if ( version_compare( $wp_version, '3.5', '>=' ) ) : ?>
                            <tr class="form-field">
                                <th scope="row"><label
                                            for="upload_handling"><?php _e( 'Upload Handling', 'njsl-networks' ); ?></label>
                                </th>
                                <td>
                                    <select name="upload_handling"
                                            id="upload_handling">
                                    ...
                                    <p class="alert">
                                        <strong><?php _e( 'WARNING', 'njsl-networks' ) ?>
                                            :</strong>
                    ...
                    <p>
                        ...
                        <a class="button"
                           href="<?php echo $this->listPage ?>"><?php _e( 'Cancel', 'njsl-networks' ); ?></a>
                    </p>
                </form>
            </div>
			<?php
	...
	function delete_site_page() {
		...
		if ( isset( $_POST['delete'] ) && isset( $_GET['id'] ) ) {
            ...
            <form method="POST" action="<?php echo $queryStr; ?>">
                <div>
                    <h2><?php _e( 'Delete Network', 'njsl-networks' ); ?>: <a
                                href="http://<?php echo $site->domain . $site->path ?>"><?php echo $site->domain . $site->path ?></a>
                    </h2>
					<?php if ( $blogs ) {
						if ( RESCUE_ORPHANED_BLOGS && ENABLE_HOLDING_SITE ) {
                            ...
                            <div id="message" class="error">
                                <p><?php _e( 'There are sites associated with this network. ', 'njsl-networks' );
									_e( 'Deleting it will move those sites to the holding network.', 'njsl-networks' ); ?></p>
                                <p>
                                    <label for="override"><?php _e( 'If you still want to delete this network, check the following box', 'njsl-networks' ); ?>
						...
						<?php } else { ?>
                            <div id="message" class="error">
                                <p><?php _e( 'There are sites associated with this network. ', 'njsl-networks' );
									_e( 'Deleting it will delete those sites as well.', 'njsl-networks' ); ?></p>
                                <p>
                                    <label for="override"><?php _e( 'If you still want to delete this network, check the following box', 'njsl-networks' ); ?>
                                        :</label> <input type="checkbox"
                                                         name="override"
	...
	function delete_multiple_site_page() {
		...
		if ( isset( $_POST['delete_multiple'] ) && isset( $_POST['deleted_sites'] ) ) {
			...
			$allsites = array_map( function ( $val ) {
				return (int) $val;
			}, $_POST['allsites'] );
                ...
                <div>
					...
					if ( $blogs ) {
						if ( RESCUE_ORPHANED_BLOGS && ENABLE_HOLDING_SITE ) {
                            ...
                            <div id="message" class="error">
                                <h3><?php _e( 'You have selected the following networks for deletion', 'njsl-networks' ); ?>
                                    :</h3>
                                <ul>
									<?php foreach ( $site as $deleted_site ) { ?>
                                        <li><input type="hidden"
                                                   name="deleted_sites[]"
                                                   value="<?php echo $deleted_site->id; ?>"/><?php echo $deleted_site->domain . $deleted_site->path ?>
                                        </li>
									<?php } ?>
                                </ul>
                                <p><?php _e( 'There are sites assigned to one or more of these networks.', 'njsl-networks' );
									_e( 'Deleting them will move these sites to the holding network.', 'njsl-networks' ); ?></p>
                                <p>
                                    <label for="override"><?php _e( 'If you still want to delete these networks, check the following box', 'njsl-networks' ); ?>
                                        :</label> <input type="checkbox"
                                                         name="override"
                                                         id="override"/></p>
                            </div>
                            ...
                            <div id="message" class="error">
                                ...
                                <p><?php _e( 'There are sites associated with one or more of these networks.', 'njsl-networks' );
									_e( 'Deleting them will delete those sites as well.', 'njsl-networks' ); ?></p>
                                <p>
                                    <label for="override"><?php _e( 'If you still want to delete these networks, check the following box', 'njsl-networks' ); ?>
                                        :</label> <input type="checkbox"
                                                         name="override"
                    ...
                    <input type="submit" name="cancel"
                           value="<?php _e( 'Cancel', 'njsl-networks' ); ?>"
                           class="button"/>
                </div>
            </form>
			<?php
		...
		if ( isset( $_POST['move'] ) && isset( $_GET['blog_id'] ) ) {
            ...
            <form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
                <table class="widefat">
                    <thead>
                    <tr>
                        <th scope="col"><?php _e( 'From', 'njsl-networks' ); ?>
                            :
                        </th>
                        <th scope="col"><label
                                    for="to"><?php _e( 'To', 'njsl-networks' ); ?>
                                :</label></th>
                    </tr>
                    </thead>
                    <tr>
				...
				<?php if ( has_action( 'add_move_blog_option' ) ) { ?>
                    <table class="widefat">
                        <thead>
                        <tr scope="col">
                            <th colspan="2"><?php _e( 'Options', 'njsl-networks' ); ?>
                                :
                            </th>
                        </tr>
                        </thead>
                ...
                <div>
                    <input type="hidden" name="from"
                           value="<?php echo $blog->site_id; ?>"/>
					<?php submit_button( __( 'Move Site', 'njsl-networks' ), 'primary', 'move', FALSE ); ?>
                    <a class="button"
                       href="<?php echo $this->sitesPage ?>"><?php _e( 'Cancel', 'njsl-networks' ); ?></a>
                </div>
            </form>
			<?php
		}
	...
	function reassign_blog_page() {
		...
		if ( isset( $_POST['reassign'] ) && isset( $_GET['id'] ) ) {
			...
			foreach ( $blogs as $key => $blog ) {
				...
				if ( ! $blog_name ) {
					$blog->name = __( 'Unknown site name', 'njsl-networks' );
				} else {
					$blog->name = stripslashes( $blog_name->option_value );
				}
			}
            ...
            <div class="wrap">
                <div class="icon32" id="icon-ms-admin"><br></div>
                <h2><?php _e( 'Assign Sites to', 'njsl-networks' ); ?>
                    : <?php echo $site->site_name . ' (' . $site->domain . $site->path . ')' ?></h2>
                <noscript>
                    <div id="message" class="updated hide-if-js">
                        <p><?php printf( __( 'Select the sites you want to assign to this network from the column at left, and click "%s."', 'njsl-networks' ), __( 'Update Assignments', 'njsl-networks' ) ); ?></p>
                    </div>
                </noscript>
                <form method="post"
                        ...
                        <tr>
                            <td>
                                <select name="from[]" id="from" multiple
                                        style="height: <?php echo $select_height ?>em; width: 98%">
									<?php
									foreach ( $blogs as $blog ) {
                            ...
                            <td>
                                <input type="button" name="unassign"
                                       id="unassign" value="<<"/><br/>
                                <input type="button" name="assign" id="assign"
                                       value=">>"/>
                            </td>
                            <td valign="top">
								<?php if ( ! ENABLE_HOLDING_SITE ) { ?>
                    ...
                    <a class="button"
                       href="<?php echo $this->listPage ?>"><?php _e( 'Cancel' ); ?></a>
                </form>
                <script type="text/javascript">
	...
	function verify_network_page() {
        ...
        <h3><?php printf( __( 'Checking <code>%s</code> table for selected Network by ID', 'njsl-networks' ), $wpdb->site ) ?>
            :</h3>
		<?php
        ...
        <h3><?php printf( __( 'Checking <code>%s</code> table for a unique combination of domain and path', 'njsl-networks' ), $wpdb->site ) ?>
            :</h3>
		<?php
        ...
        <h3><?php _e( 'Checking DNS for this network', 'njsl-networks' ) ?>
            :</h3>
		<?php
        ...
        <h3><?php _e( 'Checking hosted sites for correct Network-related values', 'njsl-networks' ) ?>
            :</h3>
		<?php
		$site_errors  = 0;
		...
		?>
        <h3><?php _e( 'Checking hosted sites for correct Network-related meta values', 'njsl-networks' ) ?>
            :</h3>
		<?php
		$site_errors = 0;
		if ( $hosted_blogs && count( $hosted_blogs ) > 0 ) {
		...
		?>
		<?php if ( is_subdomain_install() ) : ?>
            <h3><?php _e( 'Checking DNS for hosted site subdomains', 'njsl-networks' ) ?>
                :</h3>
			<?php
	...
	function save_network_options( $value, $option_name, $original_value ) {
		if ( $option_name != 'networks_per_page' ) {
			return $value;
		}
		if ( 1 < (int) $original_value && (int) $original_value <= 100 ) {
